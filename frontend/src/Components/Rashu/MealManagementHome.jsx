...
import handleSignIn from "../Raisa/handleSignIn";

const MealManagementHome = () => {
  const [showPassword, setShowPassword] = useState(false);
  const [userID, setUserID] = useState(""); // Store user ID
  const [password, setPassword] = useState(""); // Store password
  const [userType, setUserType] = useState(""); // Store user type (student or admin)
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        const response = await fetch('http://localhost:4000/api/notices/getnotices');
        if (!response.ok) {
          throw new Error('Failed to fetch notices');
        }
        const data = await response.json();
        
        // Sort notices by creation_time in descending order
        const sortedNotices = data.sort((a, b) => new Date(b.creation_time) - new Date(a.creation_time));
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-4 rounded-lg shadow-md">
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto"> {/* Add max height and scroll */}
              {loading ? (
                ...
                notices.map((notice) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500">
                      {new Date(notice.creation_time).toLocaleString()}
                    </p>
                  </div>
                ))
              ) : (
          ...
          {!currentUserId && (
            <div className="bg-white p-4 rounded-lg shadow-md">
              ...
              <form
                className="space-y-4"
                method="post"
                onSubmit={(e) => handleSignIn(e, userID, password, userType, navigate)} // Pass userType
              >
                {/* Select User Type */}
                <select
                  className="w-full p-2 rounded bg-green-100 border border-green-200"
                  value={userType} // Use state variable
